...
158 focused backend cases and app/test lint pass; independent read-only production
SQL finds 280 contamination mentions and 21 data-integrity mentions in 2025.
Application `41d3e7b` is pushed/deployed with successful Vercel/Railway statuses and
healthy web/API endpoints. All 19 final live requests return 200, and all 28
independent checks pass. The mention count improves from 25.58 to 7.61 seconds.
Chromium English, Firefox Korean and WebKit Korean complete four-turn clarification,
literal-count, follow-up and reload flows, including source focus and phone layouts.
WebKit retains the previously recorded menu-prefetch errors during hard reload;
this is not a clean console pass. Code security and nine quality CI jobs pass;
the full CI browser job is still running at the publication checkpoint. See
...
metadata requests. Metadata now uses lightweight authorization rows, loads full
source versions only for displayed citations, and computes counts across the complete
authorized result set. Calendar ranges, country filters, grouped counts, latest/oldest
ordering and repeated date follow-ups are deterministic; mixed content questions
...
recorded runs. All four Chromium startup scenarios pass. Application `521760f` is
pushed and deployed; Vercel/Railway statuses and web/API health pass. All six fresh
hosted browser checks pass (108 example pages, exact download hashes, draft handoff,
zero script errors/generation submissions). Cold sessions retain the existing
one-Retry startup recovery. Full CI browser tests remain running; other quality jobs
and code security pass. See [the refinement record](docs/assurance/interactive-interface-20260916.md).
...
nine quality-and-security jobs passed for that API release. The frontend was
published as `c98a370`, followed by prefetch correction `aa9ad63`; Vercel and both
Railway services report success. Final live checks at 02:09 UTC pass source paging,
Back, filters, empty recovery, source detail, Korean/English routes at 320/390/981/
1440px, existing archived conversation access, evidence focus return and persisted
JSON export. No JavaScript/console errors were observed; no model runs were created.
Hosted backend/package suites pass **584 tests, 9 skipped**; PostgreSQL upgrade,
restoration, contract, security and frontend checks pass. Browser CI exposed two
hosted WebKit issues: unused cancelled prefetches and a test leaving Ask before its
streamed redirect completed. Prefetches are disabled where unnecessary, and the
matrix now awaits both the Ask destination and composer before navigating onward.
The full 15-case browser gate and its evidence run against each release commit.
