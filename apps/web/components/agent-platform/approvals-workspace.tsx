...
export function ApprovalsWorkspace({ items, status }: { items: ApprovalCenterItem[]; status?: string }) {
  ...
  return <div className={styles.page}>
    ...
    <nav className={styles.filters} aria-label={text("Review filters", "검토 필터")}>
      {[undefined, "PENDING", "APPROVED", "REJECTED", "CANCELLED", "EXPIRED"].map(value => <Link key={value ?? "all"} href={value ? `/approvals?status=${value}` : "/approvals"} data-active={status === value} aria-current={status === value ? "page" : undefined}>{value ? label(value) : text("All requests", "모든 요청")}</Link>)}
    </nav>
    <section className={styles.section} aria-labelledby="review-count"><h2 id="review-count">{text(`${items.length} review requests`, `검토 요청 ${items.length}건`)}</h2>
      ...
      <div className={styles.approvalList}>{items.map(item => <article key={item.id} className={styles.approvalCard} data-status={(item.expired ? "EXPIRED" : item.status).toLowerCase()}>
        ...
        <Link className="button button--secondary" href={`/cases/${encodeURIComponent(item.caseId)}?view=${item.approvalType === "STEP_APPROVAL" ? "execution" : item.approvalType === "ARTIFACT_APPROVAL" ? "review" : "plan"}`}>{text("Open review", "검토 열기")}</Link>
