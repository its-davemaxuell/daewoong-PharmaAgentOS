...
export function OperationalOverview() {
  ...
  return (
    ...
    >
      <WorkspaceHeading title={text("Overview", "개요")}>
        <Link href="/examples">{text("Explore examples", "실행 예시 탐색")}</Link>
      ...
      <section className="continuity-continue" aria-labelledby="continue-work-title">
        <header><div><h2 id="continue-work-title">{text("Continue your work", "작업 이어가기")}</h2><p>{text("Your saved research", "저장된 내 리서치")}</p></div><Link href="/research">{text("All research", "전체 리서치")} →</Link></header>
        ...
        {!research.isPending && !research.isError && !research.data?.items.length && <div className="continuity-start"><p>{text("Start with a topic", "주제로 시작하기")}</p><div>{[
      ...
      <div className="continuity-overview-grid">
        ...
        <aside className="continuity-work">
          <section>
            ...
            <nav className="overview-inbox-shortcuts" aria-label={text("Inbox states", "받은 자료 상태")}>
              {([['new', text("New", "새 자료")], ['later', text("Later", "나중에")], ['done', text("Done", "완료")]] as const).map(([state, label]) => <Link key={state} href={`/inbox?state=${state}`}><span>{label}</span><strong>{metric(inbox.data?.counts[state])}</strong></Link>)}
            </nav>
            <p className="continuity-inbox-note">{text("Personal triage · No evidence approval", "개인 자료 정리 · 근거 승인 아님")}</p>
