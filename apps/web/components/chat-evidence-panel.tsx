...
export function ChatEvidencePanel({ citations, selected, onSelect, onClose, onFocus, disabled, focusedLetterId }: {
  ...
  return <aside className="chat-evidence-panel" aria-labelledby="evidence-panel-title" onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); onClose(); } }}>
    <header><div><span>{text("Evidence", "근거 자료")}</span><h2 id="evidence-panel-title" ref={heading} tabIndex={-1}>{text("Read the source", "원문 확인")}</h2></div>
      <button type="button" className="chat-icon-button" onClick={onClose} aria-label={text("Close evidence panel", "근거 패널 닫기")}><X size={20} /></button></header>
    <nav className="chat-evidence-panel__tabs" aria-label={text("Answer sources", "답변 출처")}>
      {citations.map((source, index) => <button key={source.id} type="button" aria-pressed={index === selected} onClick={() => onSelect(index)} aria-label={text(`Source ${index + 1}: ${source.company}`, `출처 ${index + 1}: ${source.company}`)}>{index + 1}</button>)}
    </nav>
    <div className="chat-evidence-panel__body">
      ...
      <dl><div><dt>{text("Location", "위치")}</dt><dd>{citation.anchor}</dd></div>
        <div><dt>{text("Source version", "원문 버전")}</dt><dd>{citation.sourceVersion || citation.documentVersionId || text("Current captured version", "현재 수집 버전")}</dd></div>
    ...
    <footer>
      ...
      <div><Link href={`/drug-letters/${citation.letterId}`}><FileText size={16} />{text("Letter details", "서한 상세")}</Link><a href={citation.sourceUrl} target="_blank" rel="noreferrer"><ExternalLink size={16} />FDA.gov</a></div>
    </footer>
  </aside>;
}
