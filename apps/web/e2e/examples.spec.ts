...
test("public examples filter, show real output and expose only curated downloads", async ({ page, context }, testInfo) => {
  ...
  await expect(page.getByRole("heading", { name: "See the work, from input to result." })).toBeVisible();
  ...
  await page.getByRole("heading", { name: "Quality-unit oversight, with citations" }).click();
